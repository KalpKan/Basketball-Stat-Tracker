...
| `hoops-expected-metrics.json` | Independent ground truth for those rows: per session, per UTC day (the pre-fix cross-device merge), overall, and a `dashboard` block (one row per device + UTC day, pre-2000 rows hidden, Consistency/Avg Streak on that basis) which is what the page must show since the round-1 fix. `apps/web/lib/dashboard-data.test.ts` feeds the same rows through the app's pure builder and requires identical numbers. | `../compute-expected-metrics.py` (pure Python, no SQL, no app code) |
...
Exit code 0 and the line `all counts, FG%, eFG%, swish rate, streaks, consistency and hidden shots match the ground truth`
means the numbers on the page are right for the rows in the snapshot (compared against the `dashboard` block). Take a fresh snapshot first if shots were
...
- 23 of the 95 shots carry `captured_at` on **1970-01-01** (an old iOS test sent epoch-zero timestamps).
  The raw blocks keep them; the `dashboard` block hides them (`hidden_shots: 23`) and the page says so.
- Session `90000000-0000-4000-8000-000000000100` is a single made swish: the v1 eFG% proxy gave
  **150.0%**; the bounded v2 formula (since 2026-09-18, in the view, the app and this generator) gives 100.0.
- Two devices shot on 2026-04-15 (UTC). `per_utc_day` merges them into one `day-2026-04-15` row (57 shots);
  the `dashboard` block keeps them apart (one row per device per UTC day), which is also the backend's rule.
- `consistency` on the raw-session basis is 67.6, on the cross-device UTC-day basis 85.9, on the dashboard
  basis 64.0 (4 sessions: 100.0, 75.0, 58.3, 66.7). The page shows 64.0 and names the basis on the card.
